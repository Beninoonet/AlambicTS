...
export class VoiceStateListener extends Listener {
    ...
    run(oldUser: VoiceState, newUser: VoiceState) {
        ...
        if (oldUserChannel == undefined && newUserChannel !== undefined) {
            ...
            this.container.logger.info(`${newUser.member?.user.username} a rejoint le salon : ${newUser.channel?.name} `)
        }
        else if (newUserChannel == undefined) {
            this.container.logger.info(`${oldUser.member?.user.username} a quitté le salon : ${oldUser.channel?.name} `)
        }
    }
